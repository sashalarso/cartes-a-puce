...
public class TheClient {
    ...
    private byte[] cipherGeneric( byte typeINS, byte[] challenge ) {
		...
		byte command[] =new byte[6+challenge.length];	
				
		System.arraycopy(header,0,command,0,(byte)header.length);		
		System.arraycopy(challenge,0,command,header.length,(byte)challenge.length);
		
		command[5+challenge.length]=(byte)challenge.length;
		...
		System.arraycopy(bytes, 0, result, 0,(byte)result.length);
